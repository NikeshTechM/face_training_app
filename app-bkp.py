...
# -----------------------------
def train_incremental(train_dir, model_save_path, face_data_path="face_data.pkl", n_neighbors=None, knn_algo='ball_tree'):
    X, y = [], []
    image_path = os.path.join(train_dir, 'downloaded_images')
    ...
    for label in tqdm(user_dirs, desc="Training users"):
        ...
        name_only = label.split('_', 1)[1]
        user_id = label.split('_', 1)[0]
        email = user_data[user_id]["Email"] 

        if args.with_email:
            name_only = f'{name_only}_{email}'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output-dir', type=str, default='shared_data', help='Directory for downloads and models')
    ...
    parser.add_argument('--with_email', action='store_true', help='train with email')
    ...
    image_path = os.path.join(args.output_dir, 'downloaded_images')
